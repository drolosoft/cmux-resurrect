...
- **Ghostty no longer drops a pane's tabs on the floor** — restoring a cmux layout on Ghostty silently discarded every sub-tab inside a pane: the shell, its folder and its AI session vanished from the restored workspace with nothing but a warning. Ghostty has no sub-tabs, but it does have splits, so each tab now comes back as a split beside its pane, keeping its own folder and command. The arrangement differs from the original — it can't not — but the work is all there. A backend that supports neither tabs nor splits still degrades to the old warning
- **Commands run from Ghostty act on Ghostty, even with a pinned default** — `crex restore` typed in a Ghostty window went to cmux whenever the config default was set to cmux. The "current session wins over the default" rule existed only for cmux, so the default hijacked the terminal the user was actually sitting in. It is now symmetric: the terminal crex was invoked from decides, and the configured default applies only where there is no session to infer (Alfred, cron, a plain SSH shell). A pinned default is a tie-breaker, never an override of the window you are in. `CREX_BACKEND` still wins over everything. Detection can't rely on `TERM_PROGRAM` alone, since cmux is built on Ghostty and reports `TERM_PROGRAM=ghostty` for its own shells too — the `CMUX_*` session variables disambiguate
- **A command never jumps to another terminal when yours is unreachable** — cmux can be wide open while its control socket refuses connections (Socket Control Mode disabled or password protected). Detection then reported Ghostty, so a `crex restore` typed in a cmux tab rebuilt the workspaces in Ghostty instead: the user's own session was ignored in favour of whatever else was running. The terminal you invoked crex from now keeps the command as long as its app is running, failing with "backend not reachable" rather than acting on the wrong app. The deliberate escape hatch is unchanged: a Ghostty shell holding leftover `CMUX_*` variables from a cmux that has since been *closed* still falls through to the live Ghostty
- **Restoring a pane no longer breaks on a wrapper command (GitHub #8)** — a captured command was cut off at 80 characters and saved with a trailing `...`, which is not a command at all: replaying it errored out (cmux launches Claude through a wrapper carrying a JSON `--settings` blob, and the remains failed to parse) and, worse, a cut-off path could act on the wrong target. Capture now keeps a command only when it is complete and safe to re-type, otherwise just the tool name — which AI detection then upgrades to a proper resume command. Long commands are no longer truncated at all: they're kept whole up to a much larger bound, and reduced to the tool name beyond it. Restore also repairs layouts already saved with a cut-off command, so existing layouts stop failing without needing a re-save
- **A tab's command now actually runs on restore (GitHub #8)** — commands destined for a pane's extra tabs were accepted by cmux and then silently discarded: it spawns a tab's shell only when the tab is first displayed, and input to a tab with no shell goes nowhere. Restore now selects each tab before typing into it, waits for that tab's own shell, and leaves the pane back on its first tab. Without this the per-tab session resume above would have saved commands that never executed — a live test drives a real three-tab restore and requires all three commands to have run
- **Tabs you haven't opened yet keep their own folder (GitHub #8)** — cmux spawns a tab's shell only when the tab is first displayed, and until then it reports the *workspace* directory for that tab. crex trusted that value, so saving a session before clicking through its tabs collapsed every unopened tab onto the first tab's path — the exact symptom reported after v1.26.1. Capture now prefers a live shell only when that shell is actually ready and otherwise reads the directory cmux persists per tab, which survives the lazy spawn. A running shell still wins (it knows where the user cd'd), and if the persisted state can't be read the previous behavior is used as a last resort — a save is never blocked by it. New live regression guards cover the full tab round trip and the persisted-directory source
